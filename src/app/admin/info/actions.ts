  ...
  try {
    ...
    for (let index = 1; index <= 3; index++) {
      ...
      if (isDeleted) {
        ...
        await supabaseAdmin.from('screensaver_images').delete().eq('school_id', id).eq('order_index', index)
        await supabaseAdmin.from('screensaver_images').insert({ school_id: id, image_url: uploadedUrl, order_index: index })
      }
    }
    ...
    const { data: finalData, error: updateError } = await supabaseAdmin.from('schools').update(updatePayload).eq('id', id).select();
