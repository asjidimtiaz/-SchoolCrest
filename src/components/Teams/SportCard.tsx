...
export default function SportCard({ team, primaryColor }: SportCardProps) {
  return (
    <Link
      href={`/teams/${team.id}`}
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-8 pt-12 text-white transform transition-transform duration-500">
        ...
        <div className="inline-block p-2.5 -ml-2.5 rounded-xl mb-2" style={{ backgroundColor: `${primaryColor}E6` }}>
           <h3 className="text-3xl font-black leading-[1.1] text-shadow-lg text-white break-words">{team.name}</h3>
        </div>
        
        <p className="text-[10px] font-black text-white/90 uppercase tracking-[0.3em] drop-shadow-md mb-6">{team.sport_category}</p>
