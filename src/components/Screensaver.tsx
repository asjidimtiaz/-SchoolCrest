...
const Screensaver = memo(function Screensaver({ onStart }: ScreensaverProps) {
  ...
  useEffect(() => {
    async function syncMedia() {
      ...
      let freshSponsors: string[] = [school?.sponsor_logo_1, school?.sponsor_logo_2, school?.sponsor_logo_3]
  ...
  }, [mounted, school, branding?.backgroundUrl, branding?.backgroundType]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 flex-1 w-full h-full flex flex-col items-center justify-evenly py-4 px-16 animate-fade-in text-white pointer-events-none">

        {/* Top Section: Logo with moderate padding */}
        <div className="mt-2">
          {branding.logoUrl && (
            <div className="relative h-44 w-72 drop-shadow-2xl">
              <Image
        ...
        <div className="flex flex-col items-center text-center max-w-5xl">
          <h1
            className="screensaver-title text-7xl font-black mb-4 drop-shadow-2xl tracking-tight leading-[1.1]"
            style={{ textShadow: `0 4px 20px ${branding.primaryColor}80` }}
          >
            {branding.name}
          </h1>
          ...
          {branding.tagline && (
            <p
              className="screensaver-tagline text-xl mb-8 opacity-80 drop-shadow-xl font-bold tracking-[0.4em] uppercase max-w-4xl px-4 text-center"
          ...
          <div className="animate-bounce-slow pointer-events-auto mt-4">
            <div
              className="screensaver-button px-16 py-8 rounded-full text-4xl font-black tracking-widest uppercase shadow-[0_0_80px_rgba(255,255,255,0.2)] transform hover:scale-105 active:scale-95 transition-all backdrop-blur-md border-[6px]"
              style={{
