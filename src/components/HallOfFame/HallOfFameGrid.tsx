...
export default function HallOfFameGrid({ initialData }: { initialData: Inductee[] }) {
  ...
  return (
    <>
      {/* 🏷️ Filter Categories */}
      <div className="flex flex-wrap items-center justify-center gap-3 pt-8 mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
        {categories.map((cat) => (
          <button
            key={cat}
            onClick={() => setSelectedCategory(cat)}
            className={`px-8 py-3.5 rounded-full text-xs font-black uppercase tracking-[0.2em] transition-all duration-300 active:scale-95 shadow-soft border ${selectedCategory === cat
              ? 'bg-black text-white border-black shadow-xl scale-105'
              : 'bg-white/80 backdrop-blur-md text-slate-500 border-gray-100 hover:border-gray-300'
              }`}
          >
            {cat}
          </button>
